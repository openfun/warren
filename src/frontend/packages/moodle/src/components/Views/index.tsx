...
import { useResourceViews } from "../../api";
...
/**
 * A React component for displaying daily views of selected resource within a specified date range.
 *
 * This represents an initial, simple implementation of the views plots, where all resource are presented
...
export const DailyViews: React.FC = () => {
  ...
  const { resourceMetrics: selectedData } = useResourceViews(resources, {
    since,
    until,
    ...selectedMetric,
  });
  ...
  const parseSeries = (item: ResourceMetricsResponse): Series => ({
    id: item?.id,
    name: RESOURCES.find((resource) => resource.id === item.id)?.title || "",
