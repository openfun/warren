...
export const DocumentsData: React.FC = () => {
  ...
  const { documentDownloads: downloadsData } = useDocumentDownloads(documents, { since, until });
  const { documentDownloads: downloadersData } = useDocumentDownloads(documents, {
    since,
    until,
    unique: true,
  });
